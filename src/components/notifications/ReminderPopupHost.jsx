...
function ReminderPopup({ item, onClose }) {
  return (
    <div className="pointer-events-auto w-full max-w-sm rounded-xl border border-medical-200 bg-white p-4 shadow-lg">
...
export default function ReminderPopupHost() {
  ...
  const badgeLabel = useMemo(() => {
    if (status === 'connected') return 'Reminder alerts: Live'
    if (status === 'no-token') return ''
    if (status === 'idle') return ''
    return `Reminder alerts: ${status}`
  }, [status])

  return (
    <>
      <div className="pointer-events-none fixed right-4 top-20 z-[70] flex w-[calc(100%-2rem)] max-w-sm flex-col gap-3">
        {items.map((item) => (
          <ReminderPopup
            key={item.id}
            item={item}
